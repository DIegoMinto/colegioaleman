...
@section('content')
    <div class="space-y-6">
        ...
        <x-page-header title="Administrativos" subtitle="Personal administrativo registrado en el sistema."
            :action-route="route('registro.administrativo.create')" action-label="Registrar Administrativo" />
                    ...
                    @foreach ($administrativos as $index => $administrativo)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="whitespace-nowrap text-right space-x-2">
                                <a href="{{ route('administrativos.edit', $administrativo) }}" class="btn-secondary">
                                    Editar
                                </a>
                                <form method="POST" action="{{ route('administrativos.toggle', $administrativo) }}"
                                    class="inline-block">
                                    @csrf
                                    @method('PATCH')
