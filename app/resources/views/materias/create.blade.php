...
@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-gray-200">
            <div>
                <h1 class="text-2xl font-bold text-[#7A1C1C] tracking-tight">Nueva Materia</h1>
                <p class="text-sm text-gray-500 mt-1">Registra una nueva materia en el catálogo.</p>
            </div>
            ...
            <form method="POST" action="{{ route('materias.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="nombre" name="nombre" value="{{ old('nombre') }}" class="form-input"
                        placeholder="Ej. Matemática Discreta" required>
                </div>
                ...
                <div>
                    ...
                    <select id="id_areas" name="id_areas" class="form-input bg-white" required>
                        ...
                        @foreach ($areas as $area)
                            <option value="{{ $area->id_areas }}" @selected(old('id_areas') == $area->id_areas)>
                                {{ $area->nombre }}
                            </option>
                        @endforeach
                    </select>
