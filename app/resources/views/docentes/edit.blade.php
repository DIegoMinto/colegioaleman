...
@section('content')
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <x-page-header
            ...
            subtitle="Modifica los datos personales y de cuenta."
            :back-route="route('docentes.index')"
        />

        <x-error-box />

        @php $persona = $docente->persona; $usuario = $persona->usuario; @endphp
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" id="nombres" name="nombres"
                                value="{{ old('nombres', $persona->nombres) }}" class="form-input" required>
                        </div>
                        <div>
                            <label for="ci" class="form-label">CI</label>
                            <input type="text" id="ci" name="ci" value="{{ old('ci', $persona->ci) }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="celular" name="celular"
                                value="{{ old('celular', $persona->celular) }}" class="form-input">
                        </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="email" id="email" name="email"
                                value="{{ old('email', $usuario->email) }}" class="form-input" required>
                        </div>
                        <div>
                            <label for="user" class="form-label">Usuario</label>
                            <input type="text" id="user" name="user"
                                value="{{ old('user', $usuario->user) }}" class="form-input" required>
                        </div>
                    </div>
                </div>
